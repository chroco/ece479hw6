...
void Standard_Hough( int, void* ){
  ...
  for( size_t i = 0; i < s_lines.size(); i++ ){
	...
	char temp[FILE_NAME_LENGTH*2]={'\0'};
	int i,size=sizeof("standard_hough_")/sizeof(char);
	strncpy(temp,"standard_hough_",size);
	strcat(temp,file);
	imwrite(temp, standard_hough, compression_params);
	if(!QUIET){
...
void Probabilistic_Hough( int, void* ){
  ...
  for( size_t i = 0; i < p_lines.size(); i++ ){
	...
	compression_params.push_back(95);
	char temp[FILE_NAME_LENGTH*2]={'\0'};
	int i,size=sizeof("probabilistic_hough_")/sizeof(char);
	strncpy(temp,"probabilistic_hough_",size);
	strcat(temp,file);
	imwrite(temp, probabilistic_hough, compression_params);
	if(!QUIET){
